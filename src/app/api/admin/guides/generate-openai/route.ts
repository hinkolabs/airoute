...
const THEME_RECIPES: Recipe[] = [
  { guide_type: "theme", primary_intent: "ai-tool-safety-checklist", primary_route: null, cta_type: null, cta_route_slug: null, cta_tool_slug: null, title_seed: "AI Tool Safety Checklist for Beginners" },
  { guide_type: "theme", primary_intent: "avoid-ai-scams", primary_route: null, cta_type: null, cta_route_slug: null, cta_tool_slug: null, title_seed: "How to Avoid AI Tool Scams" },
  { guide_type: "theme", primary_intent: "ai-copyright-basics", primary_route: null, cta_type: null, cta_route_slug: null, cta_tool_slug: null, title_seed: "AI Copyright Basics: What You Need to Know" },
];

const ALL_RECIPES = [...ROUTE_RECIPES, ...TOOL_RECIPES, ...THEME_RECIPES];

// =====================================================
// KR Pipeline 전용 레시피 (한국어 title_seed 포함)
// =====================================================
const KR_ROUTE_RECIPES: Recipe[] = [
  { guide_type: "route_based", primary_intent: "turn-long-videos-into-shorts", primary_route: "long-to-shorts", cta_type: "route", cta_route_slug: "long-to-shorts", cta_tool_slug: null, title_seed: "긴 영상을 유튜브 쇼츠로 변환하는 방법" },
  { guide_type: "route_based", primary_intent: "polish-shorts-and-reels", primary_route: "polish-shorts", cta_type: "route", cta_route_slug: "polish-shorts", cta_tool_slug: null, title_seed: "쇼츠와 릴스를 빠르게 다듬는 방법" },
  { guide_type: "route_based", primary_intent: "write-blog-posts-faster", primary_route: "blog-writing", cta_type: "route", cta_route_slug: "blog-writing", cta_tool_slug: null, title_seed: "AI로 블로그 글 더 빠르게 작성하기" },
  { guide_type: "route_based", primary_intent: "generate-social-media-images", primary_route: "social-images", cta_type: "route", cta_route_slug: "social-images", cta_tool_slug: null, title_seed: "AI로 SNS 이미지 생성하는 방법" },
  { guide_type: "route_based", primary_intent: "create-product-descriptions", primary_route: "product-copy", cta_type: "route", cta_route_slug: "product-copy", cta_tool_slug: null, title_seed: "AI로 상품 설명 문구 만들기" },
  { guide_type: "route_based", primary_intent: "edit-podcast-audio", primary_route: "podcast-editing", cta_type: "route", cta_route_slug: "podcast-editing", cta_tool_slug: null, title_seed: "AI 도구로 팟캐스트 오디오 편집하기" },
  { guide_type: "route_based", primary_intent: "generate-background-music", primary_route: "background-music", cta_type: "route", cta_route_slug: "background-music", cta_tool_slug: null, title_seed: "영상용 배경 음악 자동 생성하기" },
];

const KR_TOOL_RECIPES: Recipe[] = [
  { guide_type: "tool_based", primary_intent: "midjourney-beginner-guide", primary_route: null, cta_type: "tool", cta_route_slug: null, cta_tool_slug: "midjourney", title_seed: "Midjourney 입문 가이드: 처음 시작하기" },
  { guide_type: "tool_based", primary_intent: "chatgpt-writing-tips", primary_route: null, cta_type: "tool", cta_route_slug: null, cta_tool_slug: "chatgpt", title_seed: "ChatGPT 글쓰기 활용 팁" },
  { guide_type: "tool_based", primary_intent: "runway-video-editing", primary_route: null, cta_type: "tool", cta_route_slug: null, cta_tool_slug: "runway", title_seed: "Runway 영상 편집 완전 가이드" },
  { guide_type: "tool_based", primary_intent: "elevenlabs-voice-cloning", primary_route: null, cta_type: "tool", cta_route_slug: null, cta_tool_slug: "elevenlabs", title_seed: "ElevenLabs 목소리 복제 튜토리얼" },
];

const KR_THEME_RECIPES: Recipe[] = [
  { guide_type: "theme", primary_intent: "ai-tool-safety-checklist", primary_route: null, cta_type: null, cta_route_slug: null, cta_tool_slug: null, title_seed: "AI 도구 안전 사용 체크리스트" },
  { guide_type: "theme", primary_intent: "avoid-ai-scams", primary_route: null, cta_type: null, cta_route_slug: null, cta_tool_slug: null, title_seed: "AI 사기 피하는 방법" },
  { guide_type: "theme", primary_intent: "ai-copyright-basics", primary_route: null, cta_type: null, cta_route_slug: null, cta_tool_slug: null, title_seed: "AI 저작권 기초: 꼭 알아야 할 것들" },
];
...
function selectKrRecipe(): Recipe {
  const rand = Math.random() * 100;
  if (rand < 70) return KR_ROUTE_RECIPES[Math.floor(Math.random() * KR_ROUTE_RECIPES.length)];
  if (rand < 90) return KR_TOOL_RECIPES[Math.floor(Math.random() * KR_TOOL_RECIPES.length)];
...
export async function POST(req: Request) {
  ...
  try {
    ...
    console.error("[generate-openai] Auth error:", err);
  ...
  try {
    ...
    // EN pipeline: EN recipes | KR pipeline: KR recipes (독립 분기)
    ...
    // Normalize payload to satisfy guides_guide_type_link_check
    ...
    if (selectedRecipe.guide_type === 'route_based') {
      ...
      const mappedCat = categoryMap[selectedRecipe.primary_route || ''];
      ...
      const mappedCat = categoryMap[selectedRecipe.cta_tool_slug || ''];
      normalizedPayload.category = (mappedCat && VALID_CATEGORIES.includes(mappedCat)) ? mappedCat : null;
    } else {
      // theme
      normalizedPayload.primary_route = null;
      normalizedPayload.category = 'Writing';
    }
    ...
    // Fires AFTER CTA/fallback resolution, BEFORE insert (and BEFORE OpenAI cost is wasted)
    // Note: OpenAI call already happened above — dedup fires here to avoid persisting duplicates
    // =====================================================
    {
