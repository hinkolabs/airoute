...
import GuideCTA from "@/app/guides/_components/guide-cta";
...
export default async function KRGuidePage({ params }: Props) {
  ...
  // 2) KR 없으면 EN fallback
  const guide = krGuide ?? (await supabaseServerClient
    .from("guides")
    .select(selectFields)
    .eq("slug", slug)
    .eq("status", "published")
