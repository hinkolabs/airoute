...
export const metadata: Metadata = {
  title: "Airoute v2 - Find the Best AI Tools for Your Goals",
  description:
    "Too many AI tools? Choose your goal and we navigate you to the best AI workflow.",
  robots: { index: false },
};
...
const STATS = [
  ...
  { value: "50+", label: "Workflow Routes" },
  { value: "100+", label: "Expert Guides" },
];

// ─── page ────────────────────────────────────────
export default async function ENHomeV2() {
  const { data: routesData } = await supabaseServerClient
    .from("routes")
    .select(
      "id, slug, title, description, routes_i18n!left(locale, title, description)",
    )
    .eq("status", "active")
    .eq("featured", true)
  ...
  const featuredRoutes: FeaturedRoute[] = (routesData ?? []).map((r: any) => {
    ...
    return {
      ...
      description: i18n?.description ?? r.description,
    };
  });
  ...
  try {
    const { data } = await supabaseServerClient
      .from("guides")
      .select("slug, title, excerpt, guide_type, created_at")
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      <HeroV2 />
      <StatsBar />
      <PopularRoutes routes={featuredRoutes} />
    ...
    <section className="relative overflow-hidden">
      ...
      <div className="relative mx-auto max-w-3xl px-5 pb-16 pt-20 text-center sm:pt-28 sm:pb-20">
        ...
        <p className="mx-auto mt-6 max-w-xl text-base leading-relaxed text-muted-foreground sm:text-lg">
          Choose your goal. We&apos;ll show you the best AI tool combinations
          and step-by-step workflows. No more endless searching.
        </p>
...
// POPULAR ROUTES
// ═══════════════════════════════════════════════════
function PopularRoutes({ routes }: { routes: FeaturedRoute[] }) {
  return (
    <section className="px-5 py-16 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-5xl">
        <SectionHead
          ...
          sub="The most-used AI workflows"
          href="/routes"
        />
          ...
          {routes.map((r, i) => (
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                <h3 className="text-sm font-semibold text-foreground transition group-hover:text-primary sm:text-base">
                  {r.title}
                </h3>
                {r.description && (
    ...
    <section className="px-5 py-16 sm:px-6 lg:px-8 bg-muted/20">
      <div className="mx-auto max-w-5xl">
        <SectionHead
          ...
          title="Guides"
          sub="Expert guides to help you choose the right tool"
          href="/guides"
        />
    ...
    <section className="px-5 py-16 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-5xl">
        <SectionHead
          ...
          title="Categories"
          sub="Find the right AI tool for your needs"
          href="/tools"
        />
      ...
      <div className="mx-auto max-w-5xl px-5 py-10 sm:px-6 lg:px-8">
        <div className="flex flex-col items-center gap-4 text-center">
          <div className="flex gap-6 text-xs text-muted-foreground">
            <a
              href="mailto:contact@hinkolabs.com?subject=[Contact] Airoute"
              className="transition hover:text-foreground"
            >
              Contact Us
            </a>
            <a
              href="mailto:contact@hinkolabs.com?subject=[Partnership] Airoute"
              className="transition hover:text-foreground"
            >
              Partner with us
            </a>
            <a
              href="mailto:contact@hinkolabs.com?subject=[Support] Airoute"
              className="transition hover:text-foreground"
            >
              Help Center
            </a>
