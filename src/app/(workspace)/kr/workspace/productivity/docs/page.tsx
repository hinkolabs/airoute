...
import DocsSummaryClient from "./docs-summary-client";
...
export default async function KrWorkspaceProductivityDocsPage({ searchParams }: Props) {
  ...
  if (authError || !user) {
    redirect("/kr/login?next=/kr/workspace/productivity/docs");
  }
  ...
  if (membershipError || !memberships || memberships.length === 0) {
    const currentPath = encodeURIComponent("/kr/workspace/productivity/docs");
  ...
  return <DocsSummaryClient workspaceId={workspaceId} />;
}
