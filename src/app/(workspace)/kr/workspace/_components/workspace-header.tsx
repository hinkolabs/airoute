...
export default function WorkspaceHeader() {
  ...
  const { activeWorkspace, workspaces, loading: workspaceLoading, refreshWorkspace } = useWorkspace();
  ...
  const handleWorkspaceSwitch = (workspaceId: string) => {
    setIsWorkspaceMenuOpen(false);
    router.push(`/kr/workspace?ws=${workspaceId}`);
  };
  ...
  return (
    <header className="flex h-16 items-center justify-between border-b border-border bg-card px-4 lg:px-6">
      ...
      <div className="flex items-center gap-4">
        {/* Logo */}
        <Link href="/kr" className="flex items-center gap-2 shrink-0">
      ...
      <div className="flex items-center gap-3">
        {user ? (
          ...
          <>
            ...
            <Link
              href="/auth/login?next=/kr/workspace"
