...
import KrWorkspaceMarketingLandingClient from "./_components/landing-client";

type Props = {
  searchParams: Promise<{ workspace_id?: string }>;
};

export default async function KrWorkspaceMarketingLandingPage({ searchParams }: Props) {
  ...
  if (authError || !user) {
    redirect("/kr/login?next=/kr/workspace/marketing/landing");
  }
  ...
  return <KrWorkspaceMarketingLandingClient />;
}
