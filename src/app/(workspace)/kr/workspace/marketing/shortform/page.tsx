...
import ShortformClient from "./shortform-client";

type Props = {
  searchParams: Promise<{ workspace_id?: string }>;
};

export default async function ShortformPage({ searchParams }: Props) {
  ...
  if (authError || !user) {
    redirect("/kr/login?next=/kr/workspace/marketing/shortform");
  }
  ...
  if (membershipError || !memberships || memberships.length === 0) {
    const currentPath = encodeURIComponent("/kr/workspace/marketing/shortform");
  ...
  return <ShortformClient workspaceId={workspaceId} />;
}
