...
export function SaveRouteButton({ routeSlug, routeName }: SaveRouteButtonProps) {
  ...
  return (
    <div className="space-y-2">
      <button
        ...
        className={`w-full inline-flex items-center justify-center gap-2 rounded-xl px-5 py-3 text-sm font-semibold transition ${
          isSaved
            ...
            : 'border border-slate-700 bg-slate-900/50 text-slate-300 hover:border-primary/50 hover:bg-slate-800'
      ...
      {/* Limit warning */}
      {showWarning && (
        <div className="rounded-lg border border-destructive/50 bg-destructive/10 px-4 py-2 text-xs text-destructive text-center">
          {warningMessage}
        </div>
      )}
    </div>
